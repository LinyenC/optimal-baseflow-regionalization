...
% First version on 9/10/2019
% Last update: 11/20/2019

%% Functionality
% This code calculate statistics and error metrics for temporally matched pairs
%  of time series and conduct statistical significant tests for some of the metrics.
...
%  a : significant level for the statistical significance tests;
...
%  3)correlation coefficient,    4)Nash-Sutcliffe efficiency,
%  5)Kling-Gupta efficiency, and 6)coefficient of determination;
...
vr=var(TS,1,1); % Use this to match with the python jams calculation
% vr=var(TS,0,1); % Variance of target and reference
sts=[N mn vr];

%% Error metrics
ems=nan(1,6*(size(TS,2)-1));
...
% Kling-Gupta efficiency
  bias_m=mn(i)/mn(end);
  bias_sd=sqrt(vr(i)/vr(end));
  KGE=1-sqrt((CC-1).^2+(bias_m-1).^2+(bias_sd-1).^2);
% Coefficient of determination
  [~,~,~,~,r2]=regress(Ts(:,1),[Ts(:,2) ones(size(Ts,1),1)]);
  r2=r2(1);
  ems(i:(size(TS,2)-1):end)=[RMS CRMS CC(2,1) NSE KGE(2,1) r2];
...
if size(TS,2)>2 % More than 1 target series plus 1 reference series
  K=nchoosek(1:size(TS,2)-1,2); % Choose 2 target series from all the target series
  H1=nan(1,size(K,1));
  H2=nan(1,size(K,1));
  H3=nan(1,size(K,1));
  for i=1:size(K,1)
